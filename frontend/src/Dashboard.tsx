...
export const CATEGORIES = [
  ...
  { value: "shopping",  label: "Shopping",  color: "#3b82f6" },
...
const TodoModal: React.FC<ModalProps> = ({ view, initial, onClose, onSave }) => {
  ...
  const isTimeInPast = isScheduled && date === todayStr() && !!time && time < nowTime();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    if (initial) {
      await updateTodo(initial.id, { title, date, time: isScheduled ? time : "", category });
    } else {
      await createTodo({ title, date: isScheduled ? date : todayStr(), time: isScheduled ? time : "", category });
    }
    setSaving(false);
    onSave();
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <h2 className="text-lg font-semibold text-[#352323] mb-5">
          {initial ? "Edit Todo" : isScheduled ? "Schedule a Todo" : "Add a Todo"}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  style={{
                    borderColor: c.color,
                    backgroundColor: category === c.value ? c.color : "transparent",
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-gray-300 text-gray-600 py-2 rounded-lg hover:bg-black hover:text-white transition"
            >
              Cancel
            </button>
          </div>
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => { fetchTodos(); }, [view, activeFilter]);

  const fetchTodos = async () => {
    const cat = activeFilter === "all" ? undefined : activeFilter;
    const data = await getTodos(view, cat);
    setTodos(data);
  };
  ...
  const switchView   = (v: View) => { setView(v); setActiveFilter("all"); setSidebarOpen(false); };
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      `}>
        <div>
          ...
          <div className="mt-8">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Filter by tag</p>
            <div className="flex flex-col space-y-2">
              <button
                onClick={() => setActiveFilter("all")}
                className={`text-sm text-left px-2 py-1 rounded-lg transition ${activeFilter === "all" ? "bg-[#C47623] text-white" : "text-gray-600 hover:bg-gray-200"}`}
              >
                All
              </button>
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`flex items-center gap-2 text-sm text-left px-2 py-1 rounded-lg transition ${activeFilter === c.value ? "bg-gray-200 font-semibold" : "text-gray-600 hover:bg-gray-200"}`}
                >
      ...
      <main className="flex-1 bg-[#C47623] p-5 md:p-10 flex flex-col pt-16 md:pt-10 overflow-hidden">
        <div className="mb-4 text-white shrink-0">
        ...
        {view !== "completed" && (
          <button
            onClick={openAdd}
            className="mb-4 shrink-0 w-full sm:w-44 bg-[#352323] text-white py-2 rounded-lg font-medium hover:bg-black transition"
          >
            + Add Todo
          </button>
        )}
        ...
        {activeFilter !== "all" && (
          <div className="mb-3 shrink-0 flex items-center gap-2">
            <span
              className="flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full text-white"
              style={{ backgroundColor: getCat(activeFilter).color }}
            >
              {getCat(activeFilter).label}
              <button onClick={() => setActiveFilter("all")} className="ml-1 opacity-80 hover:opacity-100">✕</button>
            </span>
          </div>
        )}
